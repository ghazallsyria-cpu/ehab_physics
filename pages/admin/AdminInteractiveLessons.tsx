...
import { RefreshCw, Zap, Plus, Edit, Trash2, X, Eye, BookOpen, Send, Loader2 } from 'lucide-react';
...
const AdminInteractiveLessons: React.FC = () => {
  ...
  const handleGenerateAndSave = async () => {
      if (!aiTitle.trim() || !aiContent.trim() || !user) return;
      ...
      try {
          ...
          const newLesson: Partial<InteractiveLesson> = {
              ...
              // FIX: Replaced non-existent `points_on_complete` with logic that correctly calculates total points
              // by summing the `points` from each interaction within every scene, based on the existing type definitions.
              total_points: generatedData.scenes?.reduce((sum, scene) => {
          ...
          setGenerationError("فشل التوليد. يرجى التأكد من أن النص المدخل واضح وكافٍ، أو حاول مرة أخرى.");
      } finally {
    ...
    <div className="animate-fadeIn space-y-8 pb-20">
      <header className="flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-4">
              <button onClick={() => setShowAIModal(true)} className="bg-purple-500 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-purple-500/20 hover:scale-105 transition-all flex items-center gap-3">
                  <Plus size={16} /> إنشاء درس بالذكاء الاصطناعي
              </button>
          </div>
      </header>
                  ...
                  {isLoading ? (
                      ...
                      <tr key={lesson.id} className="border-b border-white/5 last:border-none hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="p-4 text-center flex justify-center gap-2">
                              <button onClick={() => navigate(`/interactive/${lesson.id}`)} className="p-2.5 text-blue-400 bg-blue-500/10 rounded-xl" title="معاينة"><Eye size={16}/></button>
                              <button className="p-2.5 text-amber-400 bg-amber-500/10 rounded-xl" title="تعديل"><Edit size={16}/></button>
                              <button onClick={() => handleDelete(lesson.id)} className="p-2.5 text-red-500 bg-red-500/10 rounded-xl" title="حذف"><Trash2 size={16}/></button>
                          </td>
                      </tr>
                  ))}
              </tbody>
          ...
          <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
              <div className="bg-[#0a1118] border border-purple-500/20 w-full max-w-2xl rounded-[40px] p-8 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar pr-2 space-y-4">
                      {generationError && <p className="p-3 bg-red-500/10 text-red-400 text-xs rounded-lg">{generationError}</p>}
                      <input type="text" value={aiTitle} onChange={e => setAiTitle(e.target.value)} placeholder="عنوان الدرس" className="w-full bg-black/40 p-3 rounded-lg border border-white/10 text-white font-bold"/>
                      <div className="grid grid-cols-2 gap-4">
                          <select value={aiSubject} onChange={e => setAiSubject(e.target.value)} className="w-full bg-black/40 p-3 rounded-lg border border-white/10 text-white"><option>الفيزياء</option><option>الكيمياء</option></select>
                          <select value={aiGrade} onChange={e => setAiGrade(e.target.value)} className="w-full bg-black/40 p-3 rounded-lg border border-white/10 text-white"><option>12</option><option>11</option><option>10</option></select>
                      </div>
                      <textarea
                          value={aiContent}
                          onChange={e => setAiContent(e.target.value)}
                          placeholder="الصق محتوى الدرس الخام هنا (من كتاب، ملف Word، إلخ)..."
                          className="w-full h-64 bg-black/40 p-4 rounded-lg border border-white/10 text-sm text-gray-300 resize-none outline-none focus:border-purple-400"
                      />
                  </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-white/10">
                      <button onClick={handleGenerateAndSave} disabled={isGenerating} className="w-full bg-purple-600 text-white py-4 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-purple-500 transition-all flex items-center justify-center gap-3 disabled:opacity-50">
                          ...
                          {isGenerating ? 'جاري التحليل والبناء...' : 'حوّل إلى درس تفاعلي'}
                      </button>
                  </div>
              </div>
          </div>
