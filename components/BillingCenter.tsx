...
const BillingCenter: React.FC<{ user: User; onUpdateUser: (user: User) => void }> = ({ user, onUpdateUser }) => {
  ...
  // نبدأ بالباقات الافتراضية فوراً لمنع الحالة الفارغة
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // إذا أعادت قاعدة البيانات باقات، نستخدمها، وإلا نبقى على الافتراضية
        if (plans && plans.length > 0) {
            setSubscriptionPlans(plans);
        }
      } catch (e) {
        console.error("Fetch data failed in BillingCenter", e);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-6 animate-fadeIn font-['Tajawal'] text-white text-right" dir="rtl">
      <header className="mb-20 text-center relative">
        ...
        <p className="text-gray-500 text-xl font-medium max-w-2xl mx-auto leading-relaxed">بوابتك لفتح آفاق الفيزياء المتقدمة عبر باقاتنا التعليمية.</p>
        
        <div className="mt-10 bg-emerald-500/10 border-2 border-emerald-500/20 p-6 rounded-[35px] inline-flex items-center gap-6 shadow-xl">
            ...
            <div className="text-right">
                ...
                <p className="text-xs text-gray-500 font-bold mt-1">رقمك المسجل: <span className="text-white tabular-nums">{user.phone || 'يرجى تحديثه في البروفايل'}</span></p>
            </div>
        </div>
      </header>
      ...
      {isLoading && subscriptionPlans.length === 0 ? (
          <div className="py-40 text-center flex flex-col items-center">
              ...
              <p className="text-gray-500 font-black uppercase tracking-widest text-xs">جاري فحص حالة الحساب المالي...</p>
          </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <div className="lg:col-span-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    return (
                        <div key={plan.id} className={`p-10 rounded-[55px] border-2 transition-all flex flex-col relative overflow-hidden bg-[#0a1118]/80 shadow-2xl group ${isCurrentPlan ? 'border-emerald-500/40 bg-emerald-500/[0.02]' : 'border-white/5 hover:border-amber-400/30'}`}>
                            ...
                            <div className="flex justify-between items-start mb-8 relative z-10">
                                ...
                                {isCurrentPlan ? (
                                    <span className="bg-emerald-500 text-black px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest shadow-lg">نشطة حالياً</span>
                                ) : (
                            ...
                            >
                                {isCurrentPlan ? '✓ أنت مشترك بالفعل' : plan.price === 0 ? 'ابدأ الاستخدام مجاناً' : 'اشترك الآن عبر ومض'}
                            </button>
                        </div>
                    );
                    })}
                ...
                <div className="mt-12 p-8 bg-white/[0.02] border border-white/5 rounded-[40px] flex items-start gap-6">
                    <div className="w-12 h-12 bg-amber-400/10 rounded-2xl flex items-center justify-center text-amber-400 shrink-0 border border-amber-400/20">
                        <AlertTriangle size={24}/>
                    </div>
                    <div>
                        <h4 className="text-white font-black text-sm mb-1">ملاحظة حول التفعيل</h4>
                        <p className="text-gray-500 text-xs leading-relaxed">بعد إتمام التحويل عبر "ومض"، يرجى إرسال لقطة شاشة للإيصال عبر واتساب. يتم تفعيل الحساب يدوياً من قبل الإدارة خلال أقل من 15 دقيقة.</p>
                    </div>
                </div>
            </div>
                ...
                <div className="glass-panel p-8 rounded-[45px] border-white/5 bg-[#0a1118]/90 flex flex-col shadow-3xl min-h-[500px]">
                    <div className="flex items-center justify-between mb-10 border-b border-white/5 pb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-lg font-black text-white italic">سجل العمليات</h3>
                        </div>
                        {invoices.length > 0 && <span className="bg-white/5 px-3 py-1 rounded-lg text-[9px] font-black text-gray-500">{invoices.length} سجل</span>}
                    </div>
                    
                    <div className="space-y-4 flex-1 overflow-y-auto no-scrollbar pr-1">
                        {invoices.length > 0 ? invoices.map(inv => (
                            <div key={inv.id} className="p-5 bg-white/[0.03] border border-white/5 rounded-[30px] group transition-all hover:bg-white/[0.05]">
                                <div className="flex justify-between items-center mb-4">
                                    <span className={`text-[8px] font-black px-3 py-1 rounded-full uppercase border ${inv.status === 'PAID' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>
                                        {inv.status === 'PAID' ? 'تم الدفع ✓' : 'بانتظار التأكيد ⌛'}
                                    </span>
                                ...
                                <div className="flex justify-between items-end">
                                    ...
                                    {inv.status === 'PAID' && (
                                        <button onClick={() => setSelectedInvoiceForCert(inv)} className="p-3 bg-white/5 hover:bg-white text-gray-400 hover:text-black rounded-2xl transition-all shadow-xl group/btn" title="عرض الإيصال">
                                            <Printer size={16} />
                                        </button>
                                    )}
                                </div>
                            ...
                            <div className="text-center py-24 opacity-20">
                                ...
                                <p className="text-xs font-black uppercase tracking-widest">لا توجد عمليات سابقة</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="mt-8 pt-8 border-t border-white/5">
                        <button 
                            ...
                            className="w-full flex items-center justify-center gap-3 bg-white/5 border border-white/10 text-gray-400 hover:text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all"
                        >
                            <MessageSquare size={16}/> الدعم الفني المالي
                        </button>
                    </div>
                </div>
            </div>
      ...
      <footer className="mt-24 text-center pb-10 opacity-30">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.4em]">Syrian Science Center • Kuwait Secure Payments</p>
      </footer>
    </div>
  );
};
