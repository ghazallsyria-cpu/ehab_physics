...
import { TeacherProfile, Review, User, TeacherMessage } from '../types';
...
const TeacherDirectory: React.FC<TeacherDirectoryProps> = ({ user }) => {
  ...
  const handleSelectTeacher = async (teacher: TeacherProfile) => {
    ...
    const reviewData = await dbService.getTeacherReviews(teacher.id);
  ...
  const handleSubmitReview = async () => {
    ...
    const updatedReviews = await dbService.getTeacherReviews(selectedTeacher.id);
  ...
  const handleSendMessage = async () => {
    ...
    // Privacy Filter: Redact phone numbers (8+ digits)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {teachers.map(teacher => (
          <div 
            ...
            className="glass-panel p-8 rounded-[40px] border-white/5 hover:border-[#00d2ff]/30 transition-all cursor-pointer group relative overflow-hidden flex flex-col items-center text-center"
          >
             ...
             <div className="flex gap-2 mb-6">
                {teacher.grades?.map(g => (
                    <span key={g} className="text-[9px] bg-white/5 px-2 py-1 rounded text-gray-300 font-bold border border-white/5">
                        {g === 'uni' ? 'جامعة' : `صف ${g}`}
                    </span>
                ))}
             </div>
             
             <button className="bg-white/5 border border-white/10 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest group-hover:bg-[#00d2ff] group-hover:text-black group-hover:border-[#00d2ff] transition-all">
                عرض الملف
             </button>
          </div>
        ))}
              ...
              <div className="p-8 md:p-10 border-b border-white/10 flex justify-between items-start bg-white/[0.02]">
                 <div className="flex gap-6 items-center">
                    <div className="w-20 h-20 rounded-3xl bg-gradient-to-tr from-[#00d2ff] to-blue-600 flex items-center justify-center overflow-hidden shadow-lg border border-white/10">
                        {selectedTeacher.photoUrl ? (
                            ...
                            <span className="text-4xl">{selectedTeacher.avatar}</span>
                        )}
                    </div>
                    <div>
                 ...
                 <div className="flex gap-4">
                    <button 
                        ...
                        className="bg-[#00d2ff] text-black px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-lg flex items-center gap-2"
                    >
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar grid grid-cols-1 md:grid-cols-12">
                 ...
                 <div className="md:col-span-5 p-8 border-l border-white/10 space-y-8 bg-black/20">
                       ...
                       <div className="mt-4 flex flex-wrap gap-4">
                          ...
                          <div className="bg-white/5 px-4 py-2 rounded-xl border border-white/5">
                             ...
                             <div className="flex gap-1 mt-1">
                                {selectedTeacher.grades?.map(g => (
                                    <span key={g} className="text-[8px] bg-white/10 px-1.5 rounded">{g}</span>
                                ))}
                             </div>
                          </div>
                       </div>
                    ...
                    <div className="pt-8 border-t border-white/10">
                       ...
                       <div className="space-y-4">
                          <div className="flex gap-2 justify-center bg-white/5 p-4 rounded-2xl">
                             {[1, 2, 3, 4, 5].map(star => (
                               <button 
                                 key={star} 
                                 onClick={() => setNewReview({...newReview, rating: star})}
                                 className={`text-2xl transition-transform hover:scale-125 ${star <= newReview.rating ? 'text-[#fbbf24]' : 'text-gray-600'}`}
                               >
                                 ★
                               </button>
                             ))}
                          ...
                          <textarea 
                            ...
                            className="w-full h-32 bg-white/5 border border-white/10 rounded-2xl p-4 text-sm text-white outline-none focus:border-[#00d2ff] transition-all"
                          />
                          <button 
                            onClick={handleSubmitReview}
                            disabled={isSubmitting}
                            className="w-full py-4 bg-[#00d2ff] text-black rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all shadow-lg"
                          >
                 ...
                 <div className="md:col-span-7 p-8">
                    <h4 className="text-xl font-black text-white mb-6 flex items-center gap-3">
                       آراء الطلاب <span className="bg-white/10 text-xs px-2 py-1 rounded-lg text-gray-400">{reviews.length}</span>
                    </h4>
                    <div className="space-y-4">
                       {reviews.length > 0 ? reviews.map(review => (
                         <div key={review.id} className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl animate-slideUp">
                            <div className="flex justify-between items-start mb-3">
                                <div className="flex items-center gap-3">
                                  ...
                                  <span className="font-bold text-sm text-gray-200">{review.studentName}</span>
                               </div>
                            ...
                            <p className="text-[9px] text-gray-600 mt-4 text-left">{new Date(review.timestamp).toLocaleDateString('ar-KW')}</p>
                         </div>
                       )) : (
                         <div className="py-20 text-center opacity-30">
                            <span className="text-6xl mb-4 block">💬</span>
                            <p className="font-bold text-sm">كن أول من يقيم هذا المعلم</p>
                         </div>
                       )}
                    </div>
                 </div>
        ...
        <div className="fixed inset-0 z-[250] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-[#0a1118] border border-white/10 w-full max-w-md rounded-[40px] p-8 relative shadow-3xl animate-fadeIn">
                ...
                <div className="bg-[#00d2ff]/5 border border-[#00d2ff]/20 p-4 rounded-2xl mb-6">
                    <p className="text-[10px] font-bold text-[#00d2ff] leading-relaxed">
                        🔒 الحماية النشطة: سيتم حجب أي أرقام هواتف أو وسائل تواصل خارجية تلقائياً لضمان سلامة البيئة التعليمية.
                    </p>
                </div>
                ...
                <textarea 
                    ...
                    className="w-full h-40 bg-black/40 border border-white/10 rounded-2xl p-4 text-sm text-white outline-none focus:border-[#00d2ff] mb-4"
                />

                {messageStatus && (
                    <p className={`text-xs font-bold mb-4 ${messageStatus.includes('⚠️') ? 'text-red-400' : 'text-green-400'}`}>{messageStatus}</p>
                )}

                <button 
                    onClick={handleSendMessage}
                    className="w-full py-4 bg-[#00d2ff] text-black rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-lg"
                >
                    إرسال آمن
                </button>
            </div>
