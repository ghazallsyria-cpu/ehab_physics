          ...
          <div key={q.id} className="glass-panel p-10 md:p-14 rounded-[60px] border-white/5 hover:border-[#fbbf24]/30 transition-all text-right group relative overflow-hidden bg-gradient-to-br from-white/[0.01] to-transparent">
            ...
            <div className="mb-10">
               <h4 className="text-2xl md:text-3xl font-bold leading-relaxed text-white mb-8">
                 {q.question_text.split(/(\$.*?\$)/g).map((part, i) => i % 2 === 0 ? part : <MathRenderer key={i} content={part.slice(1, -1)} />)}
               </h4>
               {q.question_latex && (
            ...
            {q.type === 'mcq' && q.choices && q.choices.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
                 {q.choices.map(choice => (
                   <div key={choice.key} className="p-7 bg-white/[0.03] border border-white/5 rounded-[30px] flex items-center justify-between group/choice hover:bg-white/[0.06] transition-all">
                      <span className="font-bold text-gray-300 group-hover/choice:text-white transition-colors">{choice.text}</span>
                      <span className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center font-black text-[#fbbf24] shadow-inner">{choice.key}</span>
                   </div>
                 ))}
              </div>
            )}
