...
class DBService {
  ...
  async getGlobalStats() {
    ...
    try {
        ...
        return { totalStudents: 0, maleStudents: 0, femaleStudents: 0, totalTeachers: 0, grade10: 0, grade11: 0, grade12: 0, uni: 0 };
    }
  }
  ...
  // --- Financial Settings (Fix V12 - Robust Upsert) ---
  ...
  // Fix: Removed async keyword to return the unsubscribe function synchronously, fixing a callable error in BillingCenter.tsx
  ...
  // --- All Other Standard Methods (Keeping logic intact) ---
  ...
  async getLoggingSettings(): Promise<LoggingSettings> { this.checkDb(); try { const snap = await getDoc(doc(db!, 'settings', 'logging')); if (snap.exists()) return snap.data() as LoggingSettings; } catch (e) {} return { logStudentProgress: true, saveAllQuizAttempts: true, logAIChatHistory: true, archiveTeacherMessages: true, forumAccessTier: 'free' }; }
  ...
  async getNotificationSettings(): Promise<NotificationSettings> { this.checkDb(); try { const snap = await getDoc(doc(db!, 'settings', 'notifications')); if (snap.exists()) return snap.data() as NotificationSettings; } catch (e) {} return { pushForLiveSessions: true, pushForGradedQuizzes: true, pushForAdminAlerts: true }; }
